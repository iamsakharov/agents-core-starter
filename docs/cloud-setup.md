...
3. Добавляй проекты в `projects.json` (или создавай через `create-project.sh` — он регистрирует
   ...
   В реестр попадают только репозитории, которыми ты владеешь единолично. Рабочие, зеркальные
   и чужие туда не добавляются: слой там работает из `~/.claude` и в git не коммитится
   (правило `project-bootstrap`, секция «Куда слой НЕ вживляется»). Даже если чужой репо
   окажется в реестре по ошибке, job `matrix` его отфильтрует по владельцу и напишет warning —
   а `sync.sh` и `create-project.sh` откажутся работать с чужим `origin`.
